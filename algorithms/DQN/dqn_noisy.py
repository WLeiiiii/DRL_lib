...
from algorithms.DQN.noisy_models import NoisyQNetworkMLP
...
class NoisyDQNAgent:
    def __init__(self, state_size, action_size, device):
        ...
        self.cfgs.noisy = True
        self.cfgs.__information__()

        # Local network is the Q-Network that will be trained
        self.local_net = NoisyQNetworkMLP(self.state_dim, self.action_dim, self.cfgs.hidden_dim).to(self.device)
        # Target network is used to compute the stable target Q-values
        self.target_net = NoisyQNetworkMLP(self.state_dim, self.action_dim, self.cfgs.hidden_dim).to(self.device)
...
if __name__ == "__main__":
    ...
    agent = NoisyDQNAgent(env.observation_space.shape, env.action_space.n, device)
